...
if __name__ == "__main__":
   ...
   parser.add_argument("--answers", default="", help="The answers to the questions for the workshop.")
   ...
   # Create workshops directory if it doesn't exist
   workshops_dir = os.path.join(os.path.dirname(folder), "workshops")
   os.makedirs(workshops_dir, exist_ok=True)
   
   # Sanitize the title for filename
   filename = sanitize_filename(title)
   
   # Create markdown file
   file_path = os.path.join(workshops_dir, f"{filename}.md")
   ...
   branch_name = f"workshop/{filename[:40]}-{timestamp}"
   ...
   commit_message = f"🔧 Add workshop: {title}"
